...
export const LayersOption: React.FC = () => {
  ...
  const [layersToRender, setLayersToRender] = useState<JSX.Element[] | null>(
    null
  );
  ...
  const toggleLayer = (layerName: string) => {
    const layers: HTMLCollectionOf<Element> =
      document.getElementsByClassName("js-mapLayer");
    ...
    Array.from(layers).forEach((layer: any) => {
      ...
      if (name === layerName) {
        ...
        const display =
          layer.style.display === "" ? computedDisplay : layer.style.display;
        const button = document.getElementById(`${name}Btn`);

        if (display === "block") {
          layer.style.display = "none";
          addNotification(
            t("notifications:notes.layers.off", {
          ...
          addNotification(
            t("notifications:notes.layers.on", {
              layer: t(`map:layers.${layerName}`)
            })
          );
        }
  ...
  return (
    <div className="layersMenu">
      ...
      <ul className="layersMenu__content t-paragraph2Light">
        <li id="backgroundBtn" onClick={toggleBackground}>
          {t("map:layers.bg")}
        </li>
        {layersToRender}
      </ul>
