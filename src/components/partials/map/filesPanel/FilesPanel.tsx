...
export const FilesPanel: React.FC = () => {
  ...
  const buildingMatrix = deepCopy(
    useSelector((state: IStore) => state.map.building.matrix)
  );
  const terrainMatrix = deepCopy(
    useSelector((state: IStore) => state.map.terrain.matrix)
  );
  const mobMatrix = deepCopy(
    useSelector((state: IStore) => state.map.mob.matrix)
  );
  const decorationMatrix = deepCopy(
    useSelector((state: IStore) => state.map.decoration.matrix)
  );
  const npcMatrix = deepCopy(
    useSelector((state: IStore) => state.map.npc.matrix)
  );
  const seMatrix = deepCopy(
    useSelector((state: IStore) => state.map.se.matrix)
  );
  ...
  const generateBookmarks = () => {
    const bookmarksToRender = bookmarks.map((bookmark: string) => {
      return (
        <li
          key={uuid()}
      ...
      <aside className="g-sidePanelWrapper" style={filesPanelStyles}>
        <div className="filesPanel">
          <nav className="filesPanel__bookmarks t-paragraph5Normal">
            <ul>{generateBookmarks()}</ul>
          </nav>

          <div className="filesPanel__imagesContainer">
            <ul>{generateImages()}</ul>
          </div>
