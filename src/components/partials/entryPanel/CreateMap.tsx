...
export const CreateMap: React.FC = () => {
  ...
  const mapSizeValidation = () => {
    ...
    if (
      typeof mapSizeX !== "number" ||
      isNaN(mapSizeX) ||
      typeof mapSizeY !== "number" ||
      isNaN(mapSizeY)
    ) {
      setValMess(t("create:map.isNotNumber"));
    } else if (
      mapSizeX >= mapConfig.map.maxSize ||
      mapSizeX < mapConfig.map.minSize ||
      mapSizeY >= mapConfig.map.maxSize ||
      mapSizeY < mapConfig.map.minSize
    ) {
      setValMess(
        t("create:map.minMax", {
