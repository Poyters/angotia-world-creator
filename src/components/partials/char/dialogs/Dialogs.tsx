...
import { findConnectedDialog } from "../../../../scripts/dialogs/findConnectedDialog";
...
export const Dialogs = () => {
  ...
  const [connectedDialogs, setConnectedDialogs] = useState<
    Array<string | number>
  >([]);
  ...
  const dialogsValidator = (beginId: string) => {
    setConnectedDialogs(findConnectedDialog(dialogsData, beginId));
  };
    ...
    <>
      {isPopupOpen
        ? ReactDOM.createPortal(
            <DialogPopup isActivePopup={setIsPopupOpen} dialogId={uuid()} />,
            document.body
          )
        : null}
      <div className="dialogs">
        <nav className="dialogs__nav">
          <ul>
            <li className="t-paragraph8Light">{t("char:dialogs.title")}</li>
