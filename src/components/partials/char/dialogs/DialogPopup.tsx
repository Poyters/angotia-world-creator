...
export const DialogPopup: React.FC<IDialogPopup> = ({
  isActivePopup,
  dialogId
}) => {
  ...
  useEffect(() => {
    if (npcText.length === 0 || !npcText) {
      setNpcTextErr(true);
    } else setNpcTextErr(false);
  }, [npcText]);

  useEffect(() => {
    if (temponaryPlayerDialogs.length === 0 || !temponaryPlayerDialogs) {
  ...
  return (
    <div className="g-container g-container--popup">
      <div role="alert" className="insertPopup insertPopup--dialog">
        <div className="g-exitBtn g-exitBtn--popup" onClick={closePopupHandler}>
          {" "}
        </div>
        ...
        <textarea value={npcText} onChange={e => setNpcText(e.target.value)} />
        {npcTextErr ? (
