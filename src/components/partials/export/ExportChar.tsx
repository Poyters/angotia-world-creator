...
export const ExportChar = ({
  ...
  const [addChar] = useMutation(CREATE_REQ_CHAR, {
    onCompleted({ createRequestedChar }) {
      // add database id to states
  ...
  const exportHandler = async () => {
    ...
    if (char.error) {
      addNotification(t("save:char.exportError"), Notification.error);
      return;
    ...
    if (char.data) {
      // Char already exists id database
