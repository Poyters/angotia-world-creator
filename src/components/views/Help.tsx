      ...
      <article className="textView__content">
        <nav className="g-sectionNav">
          ...
          <div className="g-sectionNav__elem g-sectionNav__elem--page t-paragraph6Normal">
            <Link to={`/${routesConfig.features}`}>{"help?.goToFeatures"}</Link>
          </div>
        </nav>
        <h1 className="g-sectionTitle">{"help?.title"}</h1>
        <div className="scrollWrapper">
          ...
          <section>{"help?.fs"}</section>

          <section>{"help?.import"}</section>

          <section>{"help?.vertex"}</section>

          <section>{"help?.end"}</section>
        </div>
      </article>
      <CreditsFooter
        startedYear={2018}
