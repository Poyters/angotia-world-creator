...
describe("findConnectedDialog script", () => {
  ...
  it("Give invalid dialog; beginId: 4", () => {
    expect(findConnectedDialog(dataSet, "4")).toEqual([
      "654",
      `${charConfig.invalidPrefix}4`
    ]);
  });

  it("Give invalid dialog; beginId: 2/ conditions", () => {
    expect(findConnectedDialog(dataSet, "2")).toEqual([
      "1",
      `${charConfig.invalidPrefix}2`
    ]);
  });

  it("Dialog points to itself; one dialog", () => {
    expect(findConnectedDialog(dataSet1, "4")).toEqual([
      `${charConfig.invalidPrefix}4`
    ]);
  });
});
