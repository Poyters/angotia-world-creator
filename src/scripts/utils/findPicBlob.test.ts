...
describe("findPicBlob script", () => {
  ...
  it("find blob, 1 with suffix", () => {
    expect(findPicBlob("picId=pic1", internalImages)).toBe(
      "data:image/picblob"
    );
  });

  it("find blob, 2 with suffix", () => {
    expect(findPicBlob("picId=pic2", internalImages)).toBe(
      "data:image/picblobexample"
    );
  });

  it("find blob, 3 with suffix", () => {
    expect(findPicBlob("picId=pic3", internalImages)).toBe(
  ...
  it("find blob, 2", () => {
    expect(findPicBlob("pic2", internalImages)).toBe(
      "data:image/picblobexample"
    );
  });

  it("find blob, 3", () => {
    expect(findPicBlob("pic3", internalImages)).toBe(
      "data:image/picblobexample2"
    );
  });
